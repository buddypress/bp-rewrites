<?php
/**
 * BP Rewrites Messages Component.
 *
 * @package bp-rewrites\src\bp-messages\classes
 * @since 1.0.0
 */
 ...
 * @since 1.0.0
 */
class Messages_Component extends \BP_Messages_Component {
	/**
	 * Start the messages component setup process.
	 *
	 * @since 1.0.0
	 */
	...
	/**
	 * Set up bp-messages integration with the WordPress admin bar.
	 *
	 * @since 1.5.0
	 *
	 ...
	 *
	 * This should be done inside `BP_Messages_Component::setup_admin_bar()`.
	 *
	 * @since 1.0.0
	 *
	...
	public function reset_admin_nav( $wp_admin_nav = array() ) {
		...
		if ( $wp_admin_nav ) {
			...
			$viewes_slugs = array(
				'my-account-' . $this->id . '-inbox'   => 'inbox',
				'my-account-' . $this->id . '-starred' => bp_get_messages_starred_slug(),
				'my-account-' . $this->id . '-sentbox' => 'sentbox',
				'my-account-' . $this->id . '-compose' => 'compose',
				'my-account-' . $this->id . '-notices' => 'notices',
			);
